...
import { getRoomById, getFlatmateById } from "./routes/details";
import { postRoomListing, postFlatmateListing, postPGListing, getMyListings, updateMyListing, deleteMyListing } from "./routes/listings";
import { uploadSingle, uploadMultiple } from "./routes/uploads";
...
export function createServer() {
  ...
  app.get("/api/flatmates/:id", getFlatmateById);
  ...
  app.patch("/api/my/listings/:type/:id", updateMyListing);
  app.delete("/api/my/listings/:type/:id", deleteMyListing);

  // Upload API routes
  app.post("/api/uploads", ...(uploadSingle as any));
  app.post("/api/uploads/multiple", ...(uploadMultiple as any));
