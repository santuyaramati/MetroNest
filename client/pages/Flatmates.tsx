...
import { MapPin, Search, Filter, User, Briefcase, Phone, Mail, Heart, X, Eye } from "lucide-react";
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="w-16 h-16 rounded-full overflow-hidden bg-primary/10 flex items-center justify-center">
                        <span className="text-primary font-semibold">
                          {flatmate.name?.split(' ').map((n:string)=>n[0]).join('').slice(0,2).toUpperCase()}
                        </span>
                      </div>
                      <div>
                        <h3 className="font-semibold text-lg text-gray-900 dark:text-white">
                          <Link to={`/flatmates/${flatmate.id}`} className="hover:underline">{flatmate.name}</Link>
                        </h3>
